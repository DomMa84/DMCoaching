// src/pages/api/contact.ts v17.0 (Production Ready)
import type { APIRoute } from 'astro';
import { saveContact, testConnection } from '../../lib/database.ts';

console.log('📧 Contact API v17.0 loaded');
...
export const POST: APIRoute = async ({ request }) => {
  console.log('=== CONTACT API v17.0 CALLED ===');
  
  try {
    const data: ContactData = await request.json();
    console.log('📥 Empfangene Daten v17.0:', {
      name: data.name,
      email: data.email,
      phone: data.phone,
    ...
    if (data.honeypot && data.honeypot.trim() !== '') {
      console.log('🚫 Honeypot-Schutz aktiviert v17.0 - Bot erkannt');
      return new Response(JSON.stringify({
        success: false,
        message: 'Spam erkannt',
        version: 'Contact API v17.0'
      }), {
        status: 400,
    ...
    if (errors.length > 0) {
      console.log('❌ Validierungsfehler v17.0:', errors);
      return new Response(JSON.stringify({
        success: false,
    ...
    if (isDbConnected) {
      try {
        ...
        console.log('✅ Kontakt in Datenbank gespeichert v17.0 mit ID:', contactId);
      } catch (dbError) {
        console.error('❌ Datenbankfehler v17.0:', dbError);
        // Trotzdem erfolgreich antworten, aber DB-Fehler loggen
      }
    } else {
      console.log('⚠️ Datenbank nicht verfügbar v17.0');
    }
    ...
    console.log('🎉 Kontaktanfrage erfolgreich verarbeitet v17.0');
    ...
    console.error('❌ CONTACT API ERROR v17.0:', error);
